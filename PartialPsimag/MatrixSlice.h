...
public:
  ...
  RowSlice(MatrixLikeType& m, size_t rowIndex) : mat(m), sliceRow(rowIndex) {}

  // ----- MatrixLike

  inline size_t n_col() const {
    return mat.n_col();
  }
  ...
  inline value_type& operator()(size_t rowIndex, size_t colIndex) {
    ASSERT(rowIndex == 0,
  ...
  inline const value_type& operator()(size_t rowIndex, size_t colIndex) const {
    ASSERT(rowIndex == 0,
           std::range_error("MatrixRowSlice(size_t rowIndex[bad], size_t colIndex)"));
    // Assuming mat provides it's own asserts!
    return mat(sliceRow, colIndex);
  ...
  inline const value_type& operator[](size_t componentIndex) const {
    return mat(sliceRow, componentIndex);
  }

  inline value_type operator[](size_t componentIndex) {
    return mat(sliceRow, componentIndex);
  }
};
...
template <typename MatrixLikeType>
class TransposedRowSlice : public RowSlice<MatrixLikeType> {
public:
  ...
  TransposedRowSlice(const MatrixLikeType& m, size_t rowIndex) : BaseType(m, rowIndex) {}
...
public:
  ...
  ColSlice(MatrixLikeType& m, size_t colIndex) : mat(m), sliceCol(colIndex) {}
  ...
  value_type& operator()(size_t rowIndex, size_t colIndex) {
    ASSERT(colIndex == 0, std::range_error("ColSlice::(size_t rowIndex, size_t colIndex[bad])"));
...
public:
  ...
  using BaseType::mat;
  using BaseType::sliceCol;

  TransposedColSlice(const MatrixLikeType& m, size_t colIndex) : BaseType(m, colIndex) {}

  // ----- MatrixLike

  size_t n_col() const {
    return this->mat.n_row();
  }
  size_t n_row() const {
    return 1;
  }
